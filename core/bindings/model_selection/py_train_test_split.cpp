...
void bind_train_test_split(py::module_& m) {
    m.def("train_test_split", [](
            ...
            auto split = mlite::train_test_split(
                x_view,
                y_view,
                test_size,
                random_state
            ...
            py::array_t<double> X_train({split.train_rows, split.n_features}, split.X_train.data());
            py::array_t<double> X_test({split.test_rows, split.n_features}, split.X_test.data());
            py::array_t<double> y_train(split.train_rows, split.y_train.data());
            py::array_t<double> y_test(split.test_rows, split.y_test.data());

            return py::make_tuple(X_train.attr("copy")(), X_test.attr("copy")(), y_train.attr("copy")(), y_test.attr("copy")());
        },
        py::arg("X"),
        py::arg("y"),
